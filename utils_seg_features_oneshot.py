...
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = DETCTION_THRESHOLD
cfg.SOLVER.IMS_PER_BATCH = 1
cfg.MODEL.ROI_HEADS.NUM_CLASSES = 5  # only has one class (pnumonia)
...
def on_video_ReID(videoPath, predictor, curr_tracker, curr_tracker_dets, dataset_name):
        ...
        while success:
            ...
            count = 0
            det_bboxes = np.zeros([len(names), 4], dtype=float)
            ...
            ############################################################
            raw_boxes = Boxes(torch.from_numpy(det_bboxes).cuda())
            with torch.no_grad():
                 raw_height, raw_width = image.shape[:2]
                 print("Original image size: ", (raw_height, raw_width))
        
                 # Preprocessing
                 image_raw_pre = predictor.aug.get_transform(image).apply_image(image)
                
                 print("Transformed image size: ", image_raw_pre.shape[:2])
        
                 # Scale the box
                 new_height, new_width = image_raw_pre.shape[:2]
                 scale_x = 1. * new_width / raw_width
                 scale_y = 1. * new_height / raw_height
                 #print(scale_x, scale_y)
                 oneshot_boxes = raw_boxes.clone()
                 oneshot_boxes.scale(scale_x=scale_x, scale_y=scale_y)
        
                 # ----
                 scaled_image = torch.as_tensor(image_raw_pre.astype("float32").transpose(2, 0, 1))
                 inputs = [{"image": scaled_image, "height": raw_height, "width": raw_width}]
                 scaled_images = predictor.model.preprocess_image(inputs)
        
                 # Run Backbone Res1-Res4
                 found_features = predictor.model.backbone(scaled_images.tensor)
        
                 # Run RoI head for each proposal (RoI Pooling + Res5)
                 proposal_boxes = [oneshot_boxes]
                 found_features = [found_features[f] for f in predictor.model.roi_heads.in_features]
                 print("keep:{} found_features{}:", len(names), len(det_bboxes), len(found_features))
                 
                 box_features = predictor.model.roi_heads._shared_roi_transform(found_features, proposal_boxes) 
                 '''
                 ####################################################       
            ...
            #width = x_max-x_min
            #height = y_max-y_min
            image_torch = torch.as_tensor(image.astype("float32").transpose(2, 0, 1))
            inputs = [{"image": image_torch, "height": height, "width": width}]
            trained_model = build_model(cfg)
            #DetectionCheckpointer(trained_model).load(cfg.MODEL.WEIGHTS)
            trained_model.eval()
            with torch.no_grad():
                 ...
                 features = [detectron2_features[f] for f in trained_model.roi_heads.box_in_features]
                            
                 box_features = trained_model.roi_heads.box_pooler(features, [x.proposal_boxes for x in proposals])
